...
> “FR4: pet catalog only, default-deny. Demo on 8090, shop 15 (English deck phrases): (A) **‘can you help me’** — social help, **no** ‘Still searching the catalog’ chunk; (B) **‘and what about iguanas’** — scope reply without duplicate intro; (C) **‘give me 10 dog food options’** — up to ten product cards arriving in `product_batch` chunks. Shopping + help in one message (e.g. light food for dogs… can you help?) routes to **catalog**, not FAQ. Replies stay professional — no tech manual tone (`CUSTOMER_VOICE`). Hard refresh if you tested an older build.”
