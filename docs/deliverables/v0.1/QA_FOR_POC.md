...
### Q6 — Why cap at four recommendations? Can the shopper ask for more?

**Recommended answer:**

> “**Default four** — policy P1 in `src/guardian/constraints.yaml` for clear UX aligned with the brief. **v2.1.6** adds `resolve_recommendation_count()`: the shopper can ask for more (e.g. ten options) up to **20 max**. When count > 4, `/chat/stream` emits **`product_batch`** events so the UI reveals cards in chunks of four instead of dumping everything at once. Production could make the default channel-specific; we prioritized grounded quality first.”

**Code:** `src/agents/intent_hints.py`, `src/lanes/stream.py`, `static/ui/app.js`
...
> “Same body as `/chat`, but emits events: `typing` → `chunk`/`status` → `topic` → `product_batch*` (when count > 4) or `products` → `done`. NDJSON is easy to consume from the browser without WebSockets. Status text is backend-driven (`shopper_status` from the conductor), not fake client timers. **`product_batch`** (v2.1.6) paces product cards in batches of four for larger result sets.”
...
> “Intent lane is `catalog_search`. Hybrid retrieval with an enlarged pool (up to 24 hits when a price band is detected). `price_filter.py` parses EUR ranges in multiple languages (`between`, `entre`, etc.). Then **default four** grounded picks (or more if the shopper explicitly asks, cap 20) and synthesis.”

---

### Q13b — “I’m looking for light food for my dogs… can you help me out?” — catalog or help?

**Recommended answer:**

> “**Catalog.** v2.1.6 uses `has_shopping_request()` in `intent_hints.py`: when the query contains a product need (species, food type, count) **and** a help phrase, shopping intent wins. Pure help (`can you help me` alone) stays on the social lane with no RAG progress chunk. We avoid explaining search mechanics to the shopper — `CUSTOMER_VOICE` keeps replies professional and concise.”

**Code:** `src/agents/intent_agent.py`, `src/agents/prompts.py` (`CUSTOMER_VOICE`)
...
| “Why no catalog chunk on ‘can you help me’?” | “Pure social help — phrase index + probe route before RAG; no fake ‘searching catalog’ bubble.” |
| “Why ten dog foods?” | “Shopper asked for count; `resolve_recommendation_count()` up to 20; UI gets `product_batch` chunks.” |
...
**Slide 9 demo script (English, shop 15):**

1. `can you help me` — social help, no catalog progress chunk  
2. `and what about iguanas` — scope reply, no duplicate greeting intro  
3. `give me 10 dog food options` — up to ten cards in `product_batch` waves  
...
> “PoC with hybrid RAG grounded on the catalog, shop filter, conductor that classifies before search, async FastAPI, real status streaming, default-deny guardrails, **default four / max twenty** picks with `product_batch` pacing, and **CUSTOMER_VOICE** (no tech manual to shoppers). Optimized for a 300-product demo; at scale: metadata-first retrieval, managed vector DB, global BM25, reranker, delta ingest, and Redis — without changing the API contract.”

---

## One-minute pitch (closing)

> “We delivered all five functional requirements with hybrid RAG anchored to the catalog: clean ingest with HTML stripping and shop scoping, Chroma plus BM25 plus business-signal fusion, **four grounded products by default (up to twenty on request)**, and an async FastAPI service with agentic orchestration — classify before search, stream real status and product batches to the UI, multilingual professional replies, and catalog-derived lexicon. Shopping plus help routes to catalog, not FAQ. Optimized for interview demo velocity; next steps are P0 security and structured filters, P1 fresh vectors and Redis at scale, then photo search and voice on the same `/chat` contract.”
